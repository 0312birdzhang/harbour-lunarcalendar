...
//(function() {
	...
	var _dateAdd = function(lunar, value, unit){
		if(unit == 'M'){
            return chineseLunar.monthAdd(lunar, value);
		}else{
			//转换为阳历，计算完再转为农历
            var solar = chineseLunar.lunarToSolar(lunar);
			solar = _solarAdd(solar, value, unit);
            return chineseLunar.solarToLunar(solar);
		};
	};
	...
	/*
	 农历相加
	 */
    chineseLunar.dateAdd = function(lunar, expr) {
		//分析表达式
	 ...
	 */
    chineseLunar.dateDiff = function(lunar1, lunar2, expr) {
		//计算农历月份差值
		if(expr == "M"){
            return chineseLunar.monthDiff(lunar1, lunar2);
		};

		//先转成公历，除了月份，其它的都可以按公历计算
        var solar1 = chineseLunar.lunarToSolar(lunar1);
        var solar2 = chineseLunar.lunarToSolar(lunar2);
		//再把农历转到公历
	...
	/*
	 农历月份相加
	 */
    chineseLunar.monthAdd = function(lunar, inc) {
		//如果是Date，则转换为农历
        if (lunar instanceof Date) lunar = chineseLunar.solarToLunar(lunar);
		...
		do {
			//计算当前年有多少个月
            count = chineseLunar.monthsOfYear(year);
			inc = inc + month - count;
			if (inc <= 0) {
				run = false;
		...
		//获取最后的结果年的闰月是哪一个月
        var leapMonth = chineseLunar.leapMonthOfYear(year);
		var leap = false;
		//如果闰月大于农历月，则月份减1
	 ...
	 * date1和date2允许为公历
	 */
    chineseLunar.monthDiff = function(lunar1, lunar2) {
		//如果是公历的日期格式，则转换为农历
		var count = 0;

		//如果数据类型是日期，则转换为农历
        if (lunar1 instanceof Date) lunar1 = chineseLunar.solarToLunar(lunar1);
        if (lunar2 instanceof Date) lunar2 = chineseLunar.solarToLunar(lunar2);
	 ...
	 */
    chineseLunar.daysOfMonth = function(year, month, leap) {
        ...
        var date1 = chineseLunar.lunarToSolar(year, month, 1, leap);
        var leapMonth = chineseLunar.leapMonthOfYear(year);
        ...
        var date2 = chineseLunar.lunarToSolar(year, month, 1, leap);
		return _dateDiff(date2, date1, "d");

	};

	//获取农历某一年有多少个月
    chineseLunar.monthsOfYear = function(year) {
		return MAPPING[year - MINYEAR].length - 1;
	};

	//获取农历某年的闰月是几月，
    chineseLunar.leapMonthOfYear = function(year) {
	 ...
	 */
    chineseLunar.lunarToSolar = function(year, month, day, leap) {
	...
	/*
	 ...
	 3.计入偏移量
	 */
    chineseLunar.solarToLunar = function(solar, format) {
		...
		//判断是否要格式化结果
        return (format && result) ? chineseLunar.format(result, format) : result;
	};

	//获取中国传统干支的名称
    chineseLunar.traditionalYearName = function(year) {
	...
	//获取中文的年
    chineseLunar.yearName = function(year) {
	...
	//获取中国的生肖
    chineseLunar.animalName = function(year) {
		return "鼠牛虎兔龙蛇马羊猴鸡狗猪".split("")[(year - 4) % 12];
	};

	//获取农历月的名称
    chineseLunar.monthName = function(month, traditional, leap) {
	...
	//获取农历传统天的名称
    chineseLunar.dayName = function(lunar) {
		switch (lunar) {
			case 10: return '初十';
			case 20: return '二十';
			case 30: return '三十';
	...
	//格式化农历日期，date是农历的日期
    chineseLunar.format = function(lunar, expr) {
		return expr.replace(/[TAYyMmdD]/g, function(m, i) {
			switch (m) {
				//获取传统的年
                case "T": return chineseLunar.traditionalYearName(lunar.year);
				//获取生肖
                case "A": return chineseLunar.animalName(lunar.year);
				//获取中文的年
                case "Y": return chineseLunar.yearName(lunar.year);
				//获取数字年
				case "y": return lunar.year;
				//获取月份
                case "m": return chineseLunar.monthName(lunar.month, false, lunar.leap);
				//获取传统的月
                case "M": return chineseLunar.monthName(lunar.month, true, lunar.leap);
				//获取天
                case "d": return chineseLunar.dayName(lunar.day);
				//如果是初一，则显示月，而不是显示
				case "D":
					if(lunar.day == 1){
                        return chineseLunar.monthName(lunar.month, false, lunar.leap);
					}else{
                        return chineseLunar.dayName(lunar.day);
					};
			}
		});
	};
